...
function parseSport(eventType, doc) {
	...
	if (eventType.alternateTitle !== 'ncb') {
		...
		for (i in awayScores) {
			awayScore = parseInt(awayScores[i].replace('class="regScoreAway" align="right">', '').replace('</div>', ''));
			homeScore = parseInt(homeScores[i].replace('class="regScoreHome" align="right">', '').replace('</div>', ''));
			awayTeam = awayNames[i].replace('class="regNameAway">', '').replace('</div>', '');
			homeTeam = homeNames[i].replace('class="regNameHome">', '').replace('</div>', '');
...
exports.runScript = function() {
	EventCollection.find({'inSeason' : true}, function(err, events) {
		var today = new Date();
		today = (today.getMonth() + 1) + '/' + today.getDate() + '/' + today.getFullYear();

		for (var i in events) {
			var options, path;
			if (events[i].alternateTitle !== 'ncb') {
				path = '/' + events[i] + '/caster/realtime';
				options = {
